#ifndef KPCP_SPP_LITE_H
#define KPCP_SPP_LITE_H
...
#ifdef TIMELY
    #define DELTA_TIME_INIT                     5000
    #define DELTA_TIME_MOVING_FACTOR            0.1
    #define ISSUE_TIME_AHEAD                    500000
//    #define ISSUE_TIME_AHEAD                    250000
    #define PAGE_ACCESS_THRESHOLD               1
    #define PT_ACCESS_THRESHOLD                 1
#endif
...
class SIGNATURE_TABLE {
  public:
    ...
    #ifdef TIMELY
//        uint64_t last_block_time_delta;
//        uint64_t last_block_time;
    ...
    SIGNATURE_TABLE() {
        ...
        #ifdef TIMELY
//        last_block_time_delta = DELTA_TIME_INIT;
//        last_block_time = 0;
        page_access = 0;
        #endif
...
#ifdef TIMELY
    extern uint64_t current_ST_way_index[NUM_CPUS];
    extern uint64_t current_ST_set_index[NUM_CPUS];
#endif
...
#ifdef TIMELY
    uint64_t current_ST_way_index[NUM_CPUS];
    uint64_t current_ST_set_index[NUM_CPUS];
#endif
...
int L2_ST_update(uint32_t cpu, uint64_t addr)
{
    ...
    for (match=0; match<L2_ST_WAY; match++) {
        if (table[match].valid && (table[match].tag == tag)) { // Hit 
            ...
            #ifdef TIMELY
//                #ifdef TIME_DEBUG
//                    cout<<"cpu "<<cpu<<"current cycle "<<current_core_cycle[cpu]<<" set"<<current_ST_set_index[cpu]<<" way"<<match<<", old time delta = "<<table[match].last_block_time_delta<<", new time  delta = "<<(current_core_cycle[cpu] - table[match].last_block_time)<<endl; 
//                #endif
//                table[match].last_block_time_delta = (table[match].last_block_time_delta * DELTA_TIME_MOVING_FACTOR) + ((current_core_cycle[cpu] - table[match].last_block_time) * (1 - DELTA_TIME_MOVING_FACTOR));
//                #ifdef TIME_DEBUG
//                    table[match].last_block_time_delta = current_core_cycle[cpu] - table[match].last_block_time;
//                #endif
//            table[match].last_block_time = current_core_cycle[cpu];
            current_ST_way_index[cpu] = match; 
            table[match].page_access++;
            #endif
    ...
    if (match == L2_ST_WAY) {
        for (match=0; match<L2_ST_WAY; match++) {
            if (table[match].valid == 0) { // Invalid
                ...
                #ifdef TIMELY
 //               table[match].last_block_time = current_core_cycle[cpu];
 //               table[match].last_block_time_delta = DELTA_TIME_INIT;
                current_ST_way_index[cpu] = match;
                table[match].page_access = 1;
//                    #ifdef TIME_DEBUG
//                        cout<<"### cpu "<<cpu<<" current cycle "<<current_core_cycle[cpu]<<"new sig on set "<<current_ST_set_index[cpu]<<" way "<<match<<endl;
//                    #endif
                #endif
 ...
 //       table[match].last_block_time = current_core_cycle[cpu];
        ...
        table[match].page_access = 1;
//            #ifdef TIME_DEBUG
//                cout<<"### cpu "<<cpu<<" current cycle "<<current_core_cycle[cpu]<<" current_ST_set_index "<<current_ST_set_index[cpu]<<" current_ST_way_index "<<current_ST_way_index[cpu]<<endl;
//            #endif
        #endif
...
void L2_PT_update(uint32_t cpu, int signature, int delta)
{
    ...
    for (match=0; match<L2_PT_WAY; match++) 
    {
        if (table[match].delta == delta) // Hit 
        {
            ...
            #ifdef TIMELY
            uint64_t current_core_cycle_temp = current_core_cycle[cpu];
            uint64_t new_last_block_time_delta = current_core_cycle_temp - table[match].last_block_time;
            uint64_t ddelta;
            if(new_last_block_time_delta > table[match].last_block_time_delta)
            {
                ddelta = new_last_block_time_delta - table[match].last_block_time_delta;
            }
            else
            {
                ddelta = table[match].last_block_time_delta - new_last_block_time_delta;
            }
            
            if(ddelta < 100)
            {
                table[match].c_time++;
                if(table[match].c_time > 15)
                {
                    table[match].c_time = 15;
                }
            }
            else
            {
                table[match].c_time = 1;
            }

            table[match].last_block_time_delta = new_last_block_time_delta;
            table[match].last_block_time = current_core_cycle_temp; 
            #endif
    ...
    if (match == L2_PT_WAY) // Miss
    {
        ...
        for (match=0; match<L2_PT_WAY; match++)
        {
            ...
            if(table[match].c_delta == min_val)
            {
                if(table[match].lru < table[min_idx].lru)
                {
                    min_idx = match;
                    min_val = table[match].c_delta;
                } 
